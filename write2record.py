...
def main(argv=None):
    start = datetime.now()
    with open(FLAGS.data_dir + '/train.zh.m', 'r', encoding='utf-8') as file:
        zh = file.readlines()
    with open(FLAGS.data_dir + '/train.en.m', 'r', encoding='utf-8') as file:
        en = file.readlines()
    ...
    writers = [os.path.join(FLAGS.data_dir, output_file) + '-%d-of-%d' % (thread_id, num_threads) for thread_id in range(num_threads)]
    ...
    with open(os.path.join(FLAGS.data_dir, 'vocab.zh'), 'rb') as file:
        # zh_vocab = manager.dict(pickle.load(file))
        zh_vocab = (pickle.load(file))
    with open(os.path.join(FLAGS.data_dir, 'vocab.en'), 'rb') as file:
